...
//Extract the current position and yaw from the Vicon message and update 
//the velocity. 
void vicon_data::update(const geometry_msgs::TransformStamped::ConstPtr& msg)
{
	...
	//Store the Vicon data, extract the time info from the message 
	//time stamp.
	...
	//The velocity is found between the current position and the nth 
	//previous position. Hence the next position in the storage vector 
	//will be n old.
	...
	//Find the velocity using latest measurements (only if the data is 
	//not identical and two frames have been received)
	if( (viconsPos[viconIdx] != viconsPos[nextIdx]) )
	{
		//Check if at least (viconCount-1) frames have been received. 
		//Once the (viconCount-1) frames have been stored in previous 
		//loops then the latest measurement can be used with the 
		//previous ones to find a velocity estimate.
		if (velEst_on < viconCount)
		{
			velEst_on++;
		}
		else
