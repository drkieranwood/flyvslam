...
//The readWaypointData() function is used for most of the object initialization
...
//Uses the current stored targetPos and targetLook (which have been lerp'd inside the getTagetPos() function) to find 
//the current target yaw angle. Returns the yaw angle in radians as a double. 
...
//Uses the current time, the start time and the current waypoint index to find the current target position and 
//current target look position using linear interpolation.
//Returns the current target position as a TooN vecor. Internally stores the current look position.
//Requires that at least one call is made to this function between waypoints (i.e. the waypoint transition time 
//cannot be smaller than the sampling interval)
TooN::Vector<3, double> waypoint_data::getTargetPos(ros::Time timeNow)
{
	...
	{	
		//Set the transition time. NOTE: it is not based upon the time now, but rather the time when it should have changed.
	...
	//Lerp between current index and next index, so find the next index value (wrap to first waypoint if at the end)
	...
	//Find the vector between the current waypoint and look-point and the next ones.
...
bool waypoint_data::readWaypointData(void)
{
	...
	if (myfile.is_open())
	{
		...
		//Read waypoint info from file into tmp waypoints until end of file. Ignore lines starting with a # or $ or blank.
		while (myfile.good())
		{			
			getline (myfile,line);
			...
			else if((line!="")&&(line.find("#")!=0))		//else if not a comment (starts with a #) or blank
			{
				if(myfile.good())
				{
					...
					//Read the string as a double until the first white space. Then set pEnd as the position in the string reached.
					//Store the value in the first position of the array.
					temp_read_values[0] = (double)strtod(cstr,&pEnd);

					//Moving from the previous point in the string, read the remining 6 values into the array.
					for(int ii=1 ; ii<7 ; ii++)
					{
						temp_read_values[ii] = (double)strtod(pEnd,&pEnd);
					}

					//Split the values into appropreate position, look, and time vectors.
					...
					//Once a yawpoint is added, increment the total waypoint counter
					tmp_waypointCount++;
				}
			}
